"""
Basic linear algebra operations as defined on the parameter sets of entire model.
...
class ModelParameters:
    ...
    enables the parameters of model of the same 'shape' (architecture) to be operated on as if they were 'real'
    ...
    def to(self, device):
        """
        Move the model parameters to the given device.
    ...
    def model_normalize_(self, ref_point: 'ModelParameters', order=2):
        ...
        """
        for parameter in self.parameters:
            if parameter.dim() > 1: # 维度大于1的向量才考虑规范化
        ...
        for l in range(len(self.parameters)):
            ...
            for f in range(len(self.parameters[l])):
                self.parameters[l][f] *= (ref_point.filter_norm((l, f), order)
                                          / (self.filter_norm((l, f), order)+ 1e-10))
    ...
    def flat_numpy(self) -> np.ndarray:
        """
    ...
    def flat_tensor(self) -> torch.Tensor:
        ...
        :return: a numpy array
        """
        return torch.cat([w.view(w.numel()) if w.dim() > 1 else torch.FloatTensor(w) for w in self.parameters])

    def from_numpy(self, npvec: np.array):
        idx = 0
        for w in self.parameters:
            numel = w.numel()
            w.data = torch.tensor(npvec[idx:idx + numel], dtype=w.dtype).view(w.size())
            idx += numel
        assert(idx == len(npvec))
        return self

    def from_tensor(self, tsvec: torch.Tensor):
        idx = 0
        for w in self.parameters:
            w.data = tsvec[idx:idx + w.numel()].view(w.size())
            idx += w.numel()
        assert (idx == len(tsvec))
        return self

    def _set_parameters(self, parameters: list):
        for ws, w in zip(self.parameters, parameters):
            ws.dats = w.data.clone()
